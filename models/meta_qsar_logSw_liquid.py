...
version = 1
endpoint = 'Log of solubility in water for liquid solute'
...
solute_dependencies_list = ['E', 'S', 'A', 'B', 'V', 'L', 's', 'a', 'b', 'v', 'l', 'c', 'state', 'MVliquid', 'logKaw']
...
def calculate(solutedependencies, solventdependencies, componentdependencies, solutef, solventf, componentf):
    ...
    for slvpar in ['s', 'a', 'b', 'v', 'l', 'c']:
        if solutedependencies[0][slvpar][1] in ['E', 'U']:
            ULlist.append(solutedependencies[0][slvpar][1])
        elif solutedependencies[0][slvpar][1] < 4:
            ULlist.append(solutedependencies[0][slvpar][1])
        elif solutedependencies[0][slvpar][1] == 4 and slvpar in ['s', 'a', 'b']:
            ULlist.append(1)
        elif solutedependencies[0][slvpar][1] == 4 and slvpar in ['v', 'l', 'c']:
            ULlist.append(2)
        elif solutedependencies[0][slvpar][1] == 5:
            ULlist.append(5)
        elif solutedependencies[0][slvpar][1] == 6:
            ULlist.append(3)
    ULslvqspr = aggregateUL(ULlist)
    # calculate relative solute descriptors, UL and error
    # Vri
    Vri = 1 / solutedependencies[0]['V'][0]
    Vul = 0
    Ver = 0
    # Eri
    Eri = Vri * solutedependencies[0]['E'][0]
    Eul = solutedependencies[0]['E'][1]
    if Eul == 4:
        Eul = 1
    elif Eul == 6:
        Eul = 3
    Eer = Vri * solutedependencies[0]['E'][2]
    # Sri
    Sri = Vri * solutedependencies[0]['S'][0]
    Sul = solutedependencies[0]['S'][1]
    if Sul == 4:
        Sul = 1
    elif Sul == 6:
        Sul = 3
    Ser = Vri * solutedependencies[0]['S'][2]
    # Ari
    Ari = Vri * solutedependencies[0]['A'][0]
    Aul = solutedependencies[0]['A'][1]
    if Aul == 4:
        Aul = 1
    elif Aul == 6:
        Aul = 3
    Aer = Vri * solutedependencies[0]['A'][2]
    # Bri
    Bri = Vri * solutedependencies[0]['B'][0]
    Bul = solutedependencies[0]['B'][1]
    if Bul == 4:
        Bul = 1
    elif Bul == 6:
        Bul = 3
    Ber = Vri * solutedependencies[0]['B'][2]
    # Lri
    Lri = Vri * solutedependencies[0]['L'][0]
    Lul = solutedependencies[0]['L'][1]
    if Lul == 4:
        Lul = 1
    elif Lul == 6:
        Lul = 3
    Ler = Vri * solutedependencies[0]['L'][2]
    # IAB
    IAB = (Ari * Bri)**0.5
    ...
    if type(ULslvqspr) is str or (type(ULslvqspr) is not str and type(ULslvemp) is not str and ULslvqspr < aggregateUL([ULslvemp, ULemptra])):
        ...
        logVP = solutedependencies[0]['S'][0] * solutedependencies[0]['s'][0] + \
                 solutedependencies[0]['A'][0] * solutedependencies[0]['a'][0] + \
                 solutedependencies[0]['B'][0] * solutedependencies[0]['b'][0] + \
                 solutedependencies[0]['V'][0] * solutedependencies[0]['v'][0] + \
                 solutedependencies[0]['L'][0] * solutedependencies[0]['l'][0] + \
                 solutedependencies[0]['c'][0]
        logVPUL = aggregateUL([ULslt, ULslvqspr], roundup=True)
        logVPerr = (solutedependencies[0]['V'][0]*solutedependencies[0]['v'][2])**2 + \
                    (solutedependencies[0]['L'][0]*solutedependencies[0]['l'][0])**2 * ((solutedependencies[0]['L'][2]/solutedependencies[0]['L'][0])**2 + (solutedependencies[0]['l'][2]/solutedependencies[0]['l'][0])**2) + \
                    solutedependencies[0]['c'][2]**2
        if solutedependencies[0]['S'][0] != 0 and solutedependencies[0]['s'][0] != 0:
            logVPerr += (solutedependencies[0]['S'][0]*solutedependencies[0]['s'][0])**2 * ((solutedependencies[0]['S'][2]/solutedependencies[0]['S'][0])**2 + (solutedependencies[0]['s'][2]/solutedependencies[0]['s'][0])**2)
        if solutedependencies[0]['A'][0] != 0 and solutedependencies[0]['a'][0] != 0:
            logVPerr += (solutedependencies[0]['A'][0]*solutedependencies[0]['a'][0])**2 * ((solutedependencies[0]['A'][2]/solutedependencies[0]['A'][0])**2 + (solutedependencies[0]['a'][2]/solutedependencies[0]['a'][0])**2)
        if solutedependencies[0]['B'][0] != 0 and solutedependencies[0]['b'][0] != 0:
            logVPerr += (solutedependencies[0]['B'][0]*solutedependencies[0]['b'][0])**2 * ((solutedependencies[0]['B'][2]/solutedependencies[0]['B'][0])**2 + (solutedependencies[0]['b'][2]/solutedependencies[0]['b'][0])**2)
        ...
        if type(ULslvemp) is str:
            ...
            if ULemptra > 1:
                domainnotes.append('solute descriptors out of AD of empirical correlations')
            ULslvemp = aggregateUL([ULslvemp, ULemptra])
        logVP = solutedependencies[0]['S'][0] * semp + \
                 solutedependencies[0]['A'][0] * aemp + \
                 solutedependencies[0]['B'][0] * bemp + \
                 solutedependencies[0]['V'][0] * vemp + \
                 solutedependencies[0]['L'][0] * lemp + \
                 cemp
        logVPUL = aggregateUL([ULslt, ULslvemp], roundup=True)
        logVPerr = (solutedependencies[0]['V'][0]*vemperr)**2 + \
        ...
        if solutedependencies[0]['S'][0] != 0 and semp != 0:
            logVPerr += (solutedependencies[0]['S'][0]*semp)**2 * ((solutedependencies[0]['S'][2]/solutedependencies[0]['S'][0])**2 + (semperr/semp)**2)
        if solutedependencies[0]['A'][0] != 0 and aemp != 0:
            logVPerr += (solutedependencies[0]['A'][0]*aemp)**2 * ((solutedependencies[0]['A'][2]/solutedependencies[0]['A'][0])**2 + (aemperr/aemp)**2)
        if solutedependencies[0]['B'][0] != 0 and bemp != 0:
            logVPerr += (solutedependencies[0]['B'][0]*bemp)**2 * ((solutedependencies[0]['B'][2]/solutedependencies[0]['B'][0])**2 + (bemperr/bemp)**2)
        logVPerr = logVPerr**0.5
    # convert log Ksa to logVP and cap at atmospheric pressure
    logVP = np.log10(8.31446261815324) + np.log10(298.15) - logVP - np.log10(solutedependencies[0]['MVliquid'][0]) + np.log10(1000000)
    logVPerr = (logVPerr**2 + (solutedependencies[0]['MVliquid'][2] / (solutedependencies[0]['MVliquid'][0] * np.log(10)))**2)**0.5
    # convert VP to WS by thermodynamic cycle
    logSw = logVP - np.log10(8314.46261815324) - np.log10(298.15) - solutedependencies[0]['logKaw'][0]
    # calculate total error
    logSwerr = (logVPerr**2 + solutedependencies[0]['logKaw'][2]**2)**0.5
    # get average UL
    domainnotes = [propagated_domain_notes]
    vpUL = logVPUL
    kawUL = solutedependencies[0]['logKaw'][1]
    if type(vpUL) is str or type(kawUL) is str:
        vpUL = str(vpUL)
        kawUL = str(kawUL)
        ulconcat = []
        noteconcat = []
        if 'E' in vpUL or 'E' in kawUL:
            ulconcat.append('E')
            noteconcat.append('experimental')
        if 'U' in vpUL or 'U' in kawUL:
            ulconcat.append('U')
            noteconcat.append('user')
        if vpUL.replace('E', '').replace('U', '') != '' and kawUL.replace('E', '').replace('U', '') != '':
            if '5' == vpUL.replace('E', '').replace('U', '') or '5' == kawUL.replace('E', '').replace('U', ''):
                ulconcat.append('5')
            else:
                ulconcat.append(str(int(np.ceil(((int(vpUL.replace('E', '').replace('U', '')) ** 2 + int(kawUL.replace('E', '').replace('U', '')) ** 2) / 2) ** 0.5))))
        elif vpUL.replace('E', '').replace('U', '') == '' and kawUL.replace('E', '').replace('U', '') != '':
            ulconcat.append(kawUL.replace('E', '').replace('U', ''))
        elif vpUL.replace('E', '').replace('U', '') != '' and kawUL.replace('E', '').replace('U', '') == '':
            ulconcat.append(vpUL.replace('E', '').replace('U', ''))
        else:
            ulconcat.append('')
        logSwUL = ''.join(ulconcat)
        if ulconcat[-1] != '':
            ...
            noteconcat.append('aggregate UL is in the AD')
        domainnotes.append(', '.join(noteconcat))
    else:
        if vpUL == 5 or kawUL == 5:
            logSwUL = 5
        else:
            logSwUL = int(np.ceil(((vpUL**2 + kawUL**2)/2)**0.5))
        if logSwUL <= 1:
            domainnotes.append('aggregate UL is in the AD')
        else:
            domainnotes.append('aggregate UL is out of the AD')
    if logSwUL == 'E':
        errorscale = 1
    else:
        errorscale = 1.25
