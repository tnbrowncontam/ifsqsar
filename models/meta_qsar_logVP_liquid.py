...
version = 1
endpoint = 'Log of vapor pressure of liquid'
citation = 'Brown, T. N.; '\
...
def calculate(solutedependencies, solventdependencies, componentdependencies, solutef, solventf, componentf):
    ...
    if logVP > np.log10(101325):
        domainnotes = ['Predicted vapor pressure ({}) capped at atmospheric pressure, UL set to 6; original aggregate UL: {}'.format(round(logVP, round_digits), logVPUL)] + domainnotes
        logVP = np.log10(101325)
        logVPUL = 6
    return round(logVP, round_digits), logVPUL, round(logVPerr, round_digits), '; '.join(domainnotes), citation, units, endpoint
