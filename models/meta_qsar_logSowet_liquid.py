...
version = 1
endpoint = 'Log of solubility in wet octanol for liquid or super-cooled liquid solute'
...
def calculate(solutedependencies, solventdependencies, componentdependencies, solutef, solventf, componentf):
    ...
    logSo = - 0.65 * solutedependencies[0]['S'][0] \
            + 2.67 * solutedependencies[0]['A'][0] \
            + 0.66 * solutedependencies[0]['B'][0] \
            - 1.60 * AB \
            - 1.08 * solutedependencies[0]['V'][0] \
    ...
    if solutedependencies[0]['S'][1] == 5 or solutedependencies[0]['A'][1] == 5 or solutedependencies[0]['B'][1] == 5 or solutedependencies[0]['L'][1] == 5:
        logSoUL = 5
    logSoerr = (solutedependencies[0]['V'][0] * 0.18)**2 + \
                (solutedependencies[0]['L'][0] * 0.28)**2 * ((solutedependencies[0]['L'][2] / solutedependencies[0]['L'][0])**2 + (0.04 / 0.28)**2) + \
                0.08**2
    if solutedependencies[0]['S'][0] != 0:
        logSoerr += (solutedependencies[0]['S'][0] * -0.86)**2 * ((solutedependencies[0]['S'][2] / solutedependencies[0]['S'][0])**2 + (0.12 / -0.65)**2)
    if solutedependencies[0]['A'][0] != 0:
        logSoerr += (solutedependencies[0]['A'][0] * 2.64)**2 * ((solutedependencies[0]['A'][2] / solutedependencies[0]['A'][0])**2 + (0.23 / 2.67)**2)
    if solutedependencies[0]['B'][0] != 0:
        logSoerr += (solutedependencies[0]['B'][0] * 0.10)**2 * ((solutedependencies[0]['B'][2] / solutedependencies[0]['B'][0])**2 + (0.13 / 0.66)**2)
    if AB != 0:
